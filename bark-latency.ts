...
function getBarkTweetId(payload: any): string | null {
  return payload.tags.TWEET_ID;
}

async function main() {
  const envText = await Bun.file("benchmark.env").text();
  ...
  const barkWsUrl = env.BARK_WS_URL!;
  const barkApiKey = env.BARK_API_KEY!;
  ...
  ws.on("message", (data) => {
    ...
    console.log(latency, "lat", clientTs, sft, parsed.tags.TWEET_CREATED_AT);
